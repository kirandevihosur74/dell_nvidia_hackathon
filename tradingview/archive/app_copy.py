...
def analyze_strategy():
    ...
    try:
        response = chat_completion(model='None',  # Use a valid model name
        messages=[
            {"role": "system", "content": system_prompt},
...
#         payload = {
#             "model": "None",  # Ensure you use a valid model available for this use-case
#             "messages": [
#                 {
#                     "role": "user",
#                     "content": [
...
def analyze_chart_only():
    ...
    try:
        ...
        # Send the request to OpenAI API
        response = chat_completion(
            model=request.json.get("model"),        # or hard‑code one
            messages=[
                {
                    "role": "user",
                    "content": [
...
def chat():
    try:
        ...
        selected_model = data.get('model', 'None')  # Get the selected model or default to None
...
"""
            ...
            response = chat_completion(model="None",  # Use the correct chat model
            messages=[
...
@app.route("/api/data/<ticker>/<interval>/<ema_period>/<rsi_period>", methods=["GET"])
def get_chart_data(ticker, interval, ema_period, rsi_period):
    # Your logic to fetch and return chart data here...
    return jsonify({
        "candlestick": [
